...
void main() {
  ...
  group('Парсинг items — та же логика, что выполняется внутри fetchProducts',
      () {
    ...
    test(
        'дубликаты offer_id сохраняются как есть — сервис их никак не '
        'схлопывает и не предупреждает о дубликате', () {
      final result = parseItems(jsonEncode({
        'items': [
    ...
    test(
        'null-поле у ОДНОГО элемента ломает парсинг ВСЕГО списка (не только '
