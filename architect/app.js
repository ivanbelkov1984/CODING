...
function deleteSphere(id) {
  ...
  DB.sphereLogs.filter(l => l.sphereId === id).forEach(l => tomb('sphereLogs', l.id));
...
function dedupeDigests() {
  ...
  DB.digests = (DB.digests || []).filter(d => {
    const k = digWk(d);
    if (seen[k]) { tomb('digests', d.id); removed++; return false; }
...
async function mkDig() {
  ...
  (DB.digests || []).filter(d => digWk(d) === wkKey).forEach(d => tomb('digests', d.id));
  DB.digests = (DB.digests || []).filter(d => digWk(d) !== wkKey);
  while (DB.digests.length > 19) tomb('digests', DB.digests.pop().id);
