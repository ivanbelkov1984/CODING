...
// P3: панчанга (классические списки; стандарт любого панчанга-календаря).
const GT_TITHI_NAMES = ['Пратипада', 'Двитья', 'Тритья', 'Чатуртхи', 'Панчами', 'Шашти', 'Саптами', 'Аштами', 'Навами', 'Дашами', 'Экадаши', 'Двадаши', 'Трайодаши', 'Чатурдаши'];
const GT_VARA = [['Воскресенье', 'Солнце'], ['Понедельник', 'Луна'], ['Вторник', 'Марс'], ['Среда', 'Меркурий'], ['Четверг', 'Юпитер'], ['Пятница', 'Венера'], ['Суббота', 'Сатурн']];
const GT_YOGAS = ['Вишкамбха', 'Прити', 'Аюшман', 'Саубхагья', 'Шобхана', 'Атиганда', 'Сукарма', 'Дхрити', 'Шула', 'Ганда', 'Вриддхи', 'Дхрува', 'Вьягхата', 'Харшана', 'Ваджра', 'Сиддхи', 'Вьятипата', 'Варияна', 'Паригха', 'Шива', 'Сиддха', 'Садхья', 'Шубха', 'Шукла', 'Брахма', 'Индра', 'Вайдхрити'];
const GT_KARANAS = ['Бава', 'Балава', 'Каулава', 'Тайтила', 'Гара', 'Ваниджа', 'Вишти', 'Шакуни', 'Чатушпада', 'Нага', 'Кимстугхна'];
...
export function registry() {
  ...
  // ── P3-расширение (85): панчанга (тити/вара/йога/карана) + антисции ──
  for (let n = 1; n <= 30; n++) {
    const paksha = n <= 15 ? 'шукла (растущая Луна)' : 'кришна (убывающая Луна)';
    const idx = n <= 15 ? n : n - 15;
    const name = idx === 15 ? (n <= 15 ? 'Пурнима (полнолуние)' : 'Амавасья (новолуние)') : GT_TITHI_NAMES[idx - 1];
    out.push([`tithi.${n}`, `тити рождения №${n} — ${name}`,
      `панчанга (ведический календарь): тити — лунный день, 1/30 цикла от новолуния (12° разности долгот Луны и Солнца каждый); тити №${n} — ${name}, пакша ${paksha}; тити рождения описывает лунный фон дня появления на свет`]);
  }
  GT_VARA.forEach(([day, lord], i) =>
    out.push([`vara.${i + 1}`, `вара (день недели) рождения — ${day}`,
      `панчанга: вара — день недели, управитель дня — ${lord}; ${P(lord)}; вара окрашивает день рождения архетипом своего управителя`]));
  GT_YOGAS.forEach((y, i) =>
    out.push([`yoga.${i + 1}`, `йога рождения №${i + 1} — ${y}`,
      `панчанга: йога — одна из 27 комбинаций суммы долгот Солнца и Луны (по 13°20′); йога №${i + 1} называется ${y}; это тонкий оттенок взаимодействия солнечного и лунного начал в день рождения; других фактов об этой йоге не добавляй`]));
  GT_KARANAS.forEach((k, i) =>
    out.push([`karana.${k}`, `карана рождения — ${k}`,
      `панчанга: карана — половина тити (6° разности долгот Луны и Солнца); карана ${k} — ${i < 7 ? 'подвижная (повторяется в цикле)' : 'фиксированная (встречается раз в месяц)'}; символический штрих настроения дня; других фактов не добавляй`]));
  // Пробел 1: MC (Середина Неба) в знаке — призвание и публичный образ (12).
  for (const s of SIGNS)
    out.push([`mcInSign.${s}`, `MC (Середина Неба) в знаке ${s}`,
      `MC — верхняя точка карты: призвание, публичный образ, карьерные склонности, «вершина», к которой человек естественно движется (НЕ то же самое, что Асцендент — первое впечатление); ${signFacts(s)}`]);
  for (const [pb, pr] of PLANETS)
    out.push([`antiscia.${pb}`, `антисция планеты ${pr}`,
      `антисция — зеркальное отражение долготы планеты относительно оси солнцестояний (0° Рака — 0° Козерога); точка, где ${P(pr)} звучит «отражённо», скрыто; контакт другой планеты с этой точкой создаёт неявную связь тем`]);
  return out;
}
