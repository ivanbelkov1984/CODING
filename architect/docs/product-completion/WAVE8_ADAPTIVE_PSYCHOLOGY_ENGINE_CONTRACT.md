...
добавлены `riskClass`, `amberSafe`, а каждый элемент `evidenceMetadata`
удовлетворяет явной тестируемой схеме (owner review 5238287152): `kind`,
стабильный `ref`, публикационные метаданные (`publisher`, `identifier`,
`year` — обязательны для guideline; для textbook — только проверенная
библиография, для practice_note честный null), `population`, `target`
(целевая проблема/механизм), `limitations`, `reviewedAt` и целочисленный
`evidenceVersion`. Отозванная NICE CG90 заменена актуальной **NICE NG222 —
Depression in adults: treatment and management (2022)** с сохранением
ограничения «групповая доказательность ≠ индивидуальная гарантия».
UI показывает идентификатор источника, издателя/год, дату пересмотра и версию
элемента. Персональные результаты в реестр не попадают; реестр заморожен.
...
| Файл | Объём |
|---|---|
| `tests/wave8-adaptive-engine.spec.mjs` | **93** проверки (все 38 минимальных сценариев issue #163 + схема external evidence) |
| `tests/wave8-mutation.mjs` | **17** мутаций |
...
EVENT_SOURCES, удаление без надгробия, снятие reviewedAt/evidenceVersion с
evidence-элемента, откат источника BA на отозванную CG90 — каждая роняет
ровно свой сценарий.

Focused self-review (делегация 5238310829) нашёл и закрыл два реальных
дефекта: (1) inline-обработчик исключения метода получал строковый methodId —
свой метод (OTHER) с враждебным id давал инъекцию; заменено числовыми
индексами, закреплено тестом; (2) эпизод без methodId (только семейство)
целиком выпадал из «Что помогает мне»; теперь отражается отдельным
family-профилем, не смешиваясь с реестровыми методами.
